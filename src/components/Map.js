...
const colors = { inception: "#CCCCFF", startUp: "#9999FF", expansion: "#6666FF", consolidation: "#3333CC" }
...
const Map = () =>
{
	...
	const { data, isLoading, error, isError, refetch } = useQuery(`country-${ year }`, async () =>
	{
		...
		select: (data) =>
		{
			...
			const countries = data.map((country) => ({
				...
				country_main_scores: {
					...
					digital_inclusiveness_score: Math.floor(country.fields["Digital Inclusiveness Score"] * 100),
					gender_inclusiveness_score: Math.floor(country.fields["Gender Inclusiveness Score"] * 100)
				},
	...
	useEffect(() =>
	{
		...
		currentMap.addControl(new mapboxgl.AttributionControl({ compact: false, offset: 150 }), 'bottom-right')
					...
					case "all":
						for (let i = 0; i < data.length; i++)
						{
							...
							currentMap.on("mouseenter", `${ data[i].country_name }-fill`, (e) =>
							{
								...
								ReactDOM.render(<Tooltip countryName={data[i].country_name} countryFlag={data[i].country_flag} data={data[i].country_main_scores} year={year} status={data[i].country_status} />, tooltipNode);

								customTooltip.current.setLngLat(e.lngLat).setDOMContent(tooltipNode).addTo(currentMap);
							})
	...
	}, [data, isError, isLoading, lat, lng, navigate, zoom, filter, year])
		...
		return (
			<aside className="absolute top-5 left-2 w-64 sm:hidden xs:hidden md:hidden xl:block lg:block hidden">
				<div className="my-4 space-y-2">
					<div>
						<Link to="about/scorecard"><button className="px-6 py-2 bg-brand w-full text-gray-50 rounded hover:bg-brand-400">Scorecard</button></Link>
					</div>
					<div>
						<Link to="about/scorecard"><button className="px-6 py-2 bg-brand w-full text-gray-50 rounded hover:bg-brand-400">Methodology</button></Link>
					</div>
					<div>
						<Link to="about/scorecard"><button className="px-6 py-2 bg-brand w-full text-gray-50 rounded hover:bg-brand-400">Market Development Stages</button></Link>
					</div>
				</div>
				<div className="bg-brand h-auto w-full">
					<div className="p-4">
						<div className="text-white mb-4 space-y-2">
							...
							<p className="text-sm">Lorem ipsum dolor, sit amet consectetur adipisicing elit. Id eaque assumenda neque optio, magni quo.</p>
						</div>
	...
	const loading = () =>
	{
		return <div className="absolute top-0 left-0 h-full w-full grid place-items-center bg-brand z-50">
	...
	return (
		<div className="w-full h-full relative">
			...
			<div className="w-full h-full cursor-pointer" ref={mapContainer} id="map-container">
			</div>
			...
			<aside className="absolute xl:bottom-40 lg:botttom-40 md:bottom-40  bottom-16 w-full">
				...
				<div className="flex items-center justify-between bg-gray-800 px-8 py-4 rounded text-gray-50 w-96 mx-auto">
					...
					<button onClick={handleClick} value="2020" className={year == 2020 ? `font-bold` : ""} data-tip="Data is available for 2020" data-for="2020">2020</button>
					<ReactTooltip type="dark" place="top" id="2020" />

					<button onClick={handleClick} value="2021" className={year == 2021 ? `font-bold` : ""} data-tip="Data will be available in 2022" data-for="2021">2021</button>
					<ReactTooltip type="dark" place="top" id="2021" />

					<button data-tip="No Data Available" data-for="2022">2022</button>
					<ReactTooltip type="dark" place="top" id="2022" />

					<button data-tip="No Data Available" data-for="2023">2023</button>
